...
def race_info(race):
    info = {}
    left_info   = race.find('.//ul[@class="race-headline-info race-headline-info-left"]').findall('.//li')
    right_info  = race.find('.//ul[@class="race-headline-info"]').findall('.//li')

    info['name']        = race.find('.//p[@class="map-title beta-footnote betam-caption2"]').text
    info['field_size']        = int(left_info[0].find('.//span[@class="info info beta-caption4"]').text)
    info['distance']    = left_info[1].find('.//span[@class="info beta-caption4"]').text
    info['grade']       = 'Class:' + left_info[2].find('.//span[@class="info beta-caption4"]').text
...
def load_race(link):
    ...
    if r.status_code == 200:
        ...
        r_info = race_info(doc.xpath('//div[@class="page-description module grid-header-all-sports"]')[0])
